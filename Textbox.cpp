...
void Textbox::Setup(int l_visisble, int l_charSize, int l_width, sf::Vector2f l_screenPos) {
    m_numVisible = l_visisble;
    ...
    m_backdrop.setSize(sf::Vector2f(
        l_width, (l_visisble * (l_charSize * 1.2f))));
    m_backdrop.setFillColor(sf::Color(90, 90, 90, 90));
    m_backdrop.setPosition(l_screenPos);
}

void Textbox::Add(std::string l_messages) {
    m_messages.push_back(l_messages);
